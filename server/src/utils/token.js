...
const generateAccessToken = (id, username) => {
    return jwt.sign({ id, username },
        process.env.ACCESS_TOKEN_SECRET || 'secret', { expiresIn: process.env.ACCESS_TOKEN_LIFE || '7d' }
    );
};

const generateRefreshToken = (id, username) => {
    return jwt.sign({ id, username },
        process.env.REFRESH_TOKEN_SECRET || 'secret', { expiresIn: process.env.REFRESH_TOKEN_LIFE || '30d' }
    );
}
