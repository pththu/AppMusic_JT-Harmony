...
exports.login = async (req, res) => {
  try {
    ...
    if (!isPasswordValid) {
      return res.status(200).json({ message: 'Mật khẩu không chính xác', success: false });
    }
...
exports.verifyOtpEmail = async (req, res) => {
  try {
    ...
    if (!user.otp) {
      return res.status(200).json({ message: 'No OTP found, please request a new one', success: false });
    }
    if (user.otp !== otp) {
