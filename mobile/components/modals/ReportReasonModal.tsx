...
const REPORT_REASONS = [
  ...
  {
    id: "unwanted_content",
    title: "Tôi không muốn thấy nội dung này",
    icon: "slash",
  },
];
      ...
      >
        <View className="flex-row items-center flex-1">
          <Icon
            name={item.icon}
            size={20}
            color={isSelected ? "#4f46e5" : isDark ? "#d1d5db" : "#4b5563"}
          />
          <Text
            className={`ml-3 text-base ${baseTextColor} ${isSelected ? "font-semibold text-indigo-500" : ""}`}
          >
            {item.title}
          </Text>
        </View>
        {isSelected && <Icon name="check-circle" size={20} color="#4f46e5" />}
      </TouchableOpacity>
    );
  };
    ...
    >
      <TouchableWithoutFeedback onPress={onClose}>
        <View className="flex-1 justify-end bg-black/50">
          <TouchableWithoutFeedback>
            <View className="w-full bg-white dark:bg-[#171431] rounded-t-2xl shadow-2xl p-4">
              {/* Thanh kéo (Handle) và Tiêu đề */}
              <View className="items-center mb-4">
                <View className="w-12 h-1 bg-gray-300 dark:bg-gray-600 rounded-full mb-2" />
                <Text
                  className={`text-xl font-bold text-center ${baseTextColor}`}
                >
                  Báo cáo bài viết
                </Text>
                <Text
                  className={`text-sm text-center mt-1 ${isDark ? "text-gray-400" : "text-gray-600"}`}
                >
              ...
              <View className="flex-row justify-between pt-4 border-t border-gray-200 dark:border-gray-700">
                ...
                >
                  <Text
                    className={`text-base font-bold text-center ${baseTextColor}`}
                  >
                    Hủy
                  </Text>
                </TouchableOpacity>
