...
const FollowItem: React.FC<FollowItemProps> = ({
  ...
  const followButtonText = isFollowingState ? "Hủy Theo dõi" : "Theo dõi";
    ...
    >
      ...
      {!isCurrentUser && (
        ...
        >
          {loading ? (
            <ActivityIndicator
              color={isFollowingState ? "#4F46E5" : "#4F46E5"}
            />
          ) : (
            <Text
              className={`text-sm font-semibold ${isFollowingState ? "text-red-500" : "text-white"}`}
            >
              {followButtonText}
            </Text>
          )}
...
export default function FollowListModal({
  ...
  const fetchData = async () => {
    ...
    try {
      const fetcher =
        listType === "followers" ? fetchFollowers : fetchFollowing; //
