...
const VideoSlide = ({ url }: { url: string }) => {
    ...
    return (
        <View style={{ width: IMAGE_WIDTH, height: IMAGE_HEIGHT }} className="bg-black justify-center">
            <VideoView
...
// --- SUB-COMPONENT: AUDIO SLIDE ---
const AudioSlide = ({ url }: { url: string }) => {
    ...
    return (
        <View
            ...
            className="justify-center items-center bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700"
        >
            {/* Visualizer giả lập hoặc Icon nhạc lớn */}
            <View className="mb-4 bg-gray-200 dark:bg-gray-700 p-6 rounded-full">
                <Icon name="music" size={40} color={colorScheme === 'dark' ? '#9CA3AF' : '#6B7280'} />
            </View>

            {/* Player Controls */}
            <View className="w-3/4 bg-white dark:bg-gray-900 p-4 rounded-xl shadow-sm flex-row items-center border border-gray-100 dark:border-gray-700">
                <TouchableOpacity
                    onPress={togglePlay}
                    className="w-10 h-10 bg-indigo-500 rounded-full justify-center items-center shadow-md"
                >
                    <Icon name={isPlaying ? "pause" : "play"} size={20} color="white" style={{ marginLeft: isPlaying ? 0 : 2 }} />
                </TouchableOpacity>

                <View className="ml-3 flex-1">
                    <Text className="text-xs font-bold text-gray-800 dark:text-gray-200 mb-1">Audio Clip</Text>
                    <View className="h-1 bg-gray-300 dark:bg-gray-600 rounded-full w-full overflow-hidden">
                        <View className={`h-full bg-indigo-500 ${isPlaying ? 'w-1/2' : 'w-0'}`} />
                    </View>
                </View>
            </View>
        </View>
...
// --- SUB-COMPONENT: MEDIA SWITCHER ---
const MediaSlideContent = ({ url }: { url: string }) => {
    const type = getMediaType(url);

    if (type === "video") return <VideoSlide url={url} />;
    if (type === "audio") return <AudioSlide url={url} />;
...
const PostItem = ({
    ...
    return (
        <View className="bg-white dark:bg-[#171431] p-4 mb-3 rounded-xl shadow-md shadow-gray-400 dark:shadow-black">
            ...
            {/* PHẦN GALLERY MEDIA VÀ INDICATOR (ĐÃ CẬP NHẬT) */}
            {images && images.length > 0 ? (
                <View className="mb-3">
                    <ScrollView
                        horizontal
                        ...
                        onScroll={handleScroll}
                        scrollEventThrottle={16}
                        className="rounded-xl overflow-hidden bg-gray-100 dark:bg-black"
                        style={{ height: IMAGE_HEIGHT }}
                    >
                        {images.map((url, index) => (
                            <View key={index} style={{ width: IMAGE_WIDTH, height: IMAGE_HEIGHT }}>
                                <MediaSlideContent url={url} />
                            </View>
                        ))}
                    </ScrollView>

                    {/* Indicator (Chấm tròn) chỉ hiển thị khi có nhiều hơn 1 item */}
                    {images.length > 1 && (
                        <View className="flex-row justify-center mt-2">
                            {images.map((_, index) => (
                                <View
